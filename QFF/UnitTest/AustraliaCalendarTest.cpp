...
TEST_F(AustraliaCalendarTestFixture, isBusinessDayTest_Weekednd) {
  EXPECT_FALSE(ausCalendar.isBusinessDay(date(2019, 4, 20)));
}

TEST_F(AustraliaCalendarTestFixture, isBusinessDayTest_Holiday) {
  EXPECT_FALSE(ausCalendar.isBusinessDay(date(2019, 4, 19)));
}

TEST_P(AustraliaCalendarTestFixture, isHolidayTest_Holidays) {
  date holiday = GetParam();
  EXPECT_TRUE(ausCalendar.isHoliday(holiday));
}

INSTANTIATE_TEST_CASE_P(
    Holidays, AustraliaCalendarTestFixture,
    ::testing::Values(date(2018, 1, 1), date(2018, 1, 26), date(2018, 3, 30),
                      ...
                      date(2019, 12, 25), date(2019, 12, 26),
                      date(2020, 1, 1)));
