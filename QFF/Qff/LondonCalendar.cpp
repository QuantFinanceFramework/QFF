...
date LondonCalendar::AdjustHoliday(const date& holiday) const {
...
date LondonCalendar::NewYearsDay(int year) const {
  return AdjustHoliday(date(year, 1, 1));
}

date LondonCalendar::GoodFriday(int year) {
  return EasterMonday(year) - boost::gregorian::days(3);
}

date LondonCalendar::EasterMonday(int year) {
...
date LondonCalendar::SpringBankHoliday(int year) {
  return previous_weekday(date(year, 5, 31), boost::gregorian::greg_weekday(
                                                 boost::date_time::Monday));
}

date LondonCalendar::SummerBankHoliday(int year) {
  return previous_weekday(date(year, 8, 31), boost::gregorian::greg_weekday(
                                                 boost::date_time::Monday));
}

date LondonCalendar::ChristmasDay(int year) const {
  return AdjustHoliday(date(year, 12, 25));
}

date LondonCalendar::BoxingDay(int year) const {
